...
(import-macros {: set! : local-set!} :themis.opt)
...
;;; Terminal
;;; ========
(augroup! terminal-options (clear!)
...
;;; Disable 'wrap'
;;; ==============
(augroup! disable-wrap (clear!)
          (autocmd! FileType [clojure fennel lisp] `(local-set! nowrap)))

;;; ===========================
;;; Properly open files with gf
;;; ===========================
(augroup! properly-open-files-with-gf (clear!) ;; Fennel
...
;;; Set 'formatprg'
;;; ===============
(augroup! set-formatprg (clear!) ;; Fennel
          (autocmd! FileType [fennel]
                    `(local-set! formatprg
                                 "fnlfmt - | tail -r | tail -n +2 | tail -r")))
